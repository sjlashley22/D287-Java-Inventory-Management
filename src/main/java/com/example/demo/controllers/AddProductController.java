        ...
        if(bindingResult.hasErrors()){
            ...
            if(product.getId()!=0) {
                ...
                if (product.getInv() - product2.getInv() > 0) {
                    ...
                    int decrease = product.getInv() - product2.getInv();

                    for (Part p : product2.getParts()) {
                        if (p.getInv() - decrease < p.getMin()) {
                            bindingResult.reject("error.product", "Not enough inventory for part: " + p.getName());
                        }
                    }
                    ...
                    for (Part p : product2.getParts()) {p.setInv(p.getInv() - decrease);
                        partService1.save(p);
                    }
                }
            }
